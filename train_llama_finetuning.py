...
model_name = "hf-internal-testing/llama-3b"
...
print(f"Accuracy before fine-tuning (LLaMA): {before_accuracy}")

# Evaluate after fine-tuning
fine_tuned_model_path = "./results_llama/checkpoint-best"
model = LlamaForSequenceClassification.from_pretrained(fine_tuned_model_path)
trainer = Trainer(model=model)
outputs = trainer.predict(train_dataset)
after_accuracy = accuracy_score(train_data['label'], outputs.predictions.argmax(axis=1))
print(f"Accuracy after fine-tuning (LLaMA): {after_accuracy}")
